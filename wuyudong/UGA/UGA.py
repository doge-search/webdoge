...
class ProfList:
    ...
    def getProfList(self):
        page = self.getPage()
        regex = '<div class="views-row.*?">.*?<div class=".*?photo">.*?<img.*?src="(.*?)".*?</div>.*?<div class=".*?last-name">.*?<a href="(.*?)">(.*?)</a></div>.*?<div class=".*?email">(.*?)<script.*?<div class=".*?phone-number">.*?<div class="field-content">(.*?)</div>'
        #regex = '<tr>.*?<img src=\"(.*?)\".*?</tr>'
        ...
        for item in myItems:
            ...
            ProfName = item[2]
            ProfPhotoUrl = item[0]
            ProfPUrl = "http://cs.uga.edu" + item[1]
            ProfTitle = ""
            ProfArea = ""
            ProfOffice = ""
            ProfPhone = item[4]
...
baseURL = 'http://cs.uga.edu/directory/regular-faculty'
pl = ProfList(baseURL)
pl.outPutProf()
# pl.getPage()
# pl.getProfList()
