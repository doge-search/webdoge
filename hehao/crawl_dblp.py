...
class Author(LazyAPIData):
	...
	def load_data(self):
		try: resp = requests.get(DBLP_PERSON_URL.format(urlpt=self.urlpt), timeout=1000)
...
class Publication(LazyAPIData):
	...
	def load_data(self):
		try: resp = requests.get(DBLP_PUBLICATION_URL.format(key=self.key), timeout=1000)
		except Exception as e:
			print "time out!"
			raise ValueError
...
def dblp_search(author_str):
	try: resp = requests.get(DBLP_AUTHOR_SEARCH_URL, params={'xauthor':author_str}, timeout=1000)
	except Exception as e:
		print "time out in search!"
		raise ValueError
	#TODO error handling
...
schools = ['BUFFALO', 'CASE', 'COLOSTATE', 'Cornell', 'GMU', 'iastate', 'MSU', 'NEU', 'NYU', 'oregonstate', 'Princeton', 'SU', 'TUFTS', 'UCR', 'UIC', 'UIOWA', 'UND', 'UOREGON', 'UTEXAS', 'UWashington', 'VU'] 
def search(ini_name):
	name = ini_name.strip()
	nname = name.split(',')
	errorflag = False
